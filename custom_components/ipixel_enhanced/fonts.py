...
@lru_cache(maxsize=1)
def _scan_default_fonts() -> tuple[str, ...]:
    """Scan the default font locations once and cache the result.

    The disk scan (glob/rglob) blocks, so this is cached and should be warmed
    from an executor (see async_setup_entry) before any entity reads it.
    """
    return tuple(_scan_fonts(get_font_locations()))
...
def get_available_fonts(locations: list[Path] | None = None) -> list[str]:
    ...
    """
    if locations is None:
        return list(_scan_default_fonts())
    return _scan_fonts(locations)


def _scan_fonts(locations: list[Path]) -> list[str]:
    """Scan the given directories for TTF/OTF font files."""
    fonts = set()
